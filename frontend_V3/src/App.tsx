...
function notificationMeta(type: string) {
  ...
  if (normalized.includes('comment')) return { path: '/activity', action: 'See comment', type: 'reply' }
  if (normalized.includes('like')) return { path: '/activity', action: 'See activity', type: 'like' }
...
function CommentRow({
  ...
  const { currentUser, deleteComment, reportComment } = useApp()
            ...
            <div className="service-kicker-row">
              ...
              {isOwnService ? (
                <div className="service-owner-menu" ref={serviceMenuRef}>
                  ...
                  {serviceMenuOpen ? (
                    <div className="floating-menu service-options-menu">
...
function ProfilePage() {
  ...
  const navigate = useNavigate()
  const { currentUser, users, posts, services, followedUserIds, updateProfile, openCreate, startConversation, togglePostLike, togglePostSave } = useApp()
        ...
        <div className="utility-list">
          {likedPosts.slice(0, 4).map((post) => (
            <Link key={post.id} to={`/p/${post.id}`}>
              <Heart size={18} />
              <span>{post.caption || 'Post without caption'}</span>
            </Link>
          ))}
          {!likedPosts.length ? (
            <div><Sparkles size={18} /><span>No liked posts yet.</span></div>
          ) : null}
        </div>
      </section>
    </AppShell>
